...
public class Payroll extends CFDIBill {
    ...
    public Payroll(Worker worker, Restaurant restaurant) {
        super(restaurant.getStreet(),type,restaurant.getName(),restaurant.getNIF(),worker.getJob().toString(),worker.getNIF(),worker.getSalary(), BillSettings.getConcept("Payroll"));
    }
