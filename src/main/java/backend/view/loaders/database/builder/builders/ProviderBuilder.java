...
public class ProviderBuilder extends Builder<Provider> {
    ...
    protected List<Object> getRow(Provider provider) {
        return Arrays.asList(new Object[]{provider.getNIF()
                ,provider.getCompanyName(),provider.getOwnerName()
                ,provider.getStreet(),provider.getCreationDate()
                ,provider.getTelephoneNumber()});
    }

    @Override
    protected Provider getItem(Object[] parameters) {
        return new Provider((int)parameters[0],(String) parameters[1],(String)parameters[2],(String)parameters[3],(String)parameters[4],(String)parameters[5]);
    }
}
